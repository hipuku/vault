...
export function formatSize(px: number, unit: SizeUnit): string {
  switch (unit) {
    case 'px':
      return `${trim(px)}px`
    case 'rem':
      return `${trim(px / ROOT_PX)}rem`
    case 'pt':
      return `${trim(px * 0.75)}pt`
  }
}
...
export function formatLineHeight(value: string, sizePx: number, unit: LineHeightUnit): string {
  ...
  switch (unit) {
    case 'unitless':
      return trim(mult)
    case 'px':
      return `${trim(mult * sizePx)}px`
    case '%':
      return `${trim(mult * 100)}%`
  }
}
 ...
 *                silently drops.
 */
export function formatTracking(value: string, sizePx: number, unit: TrackingUnit, forCss = false): string {
  ...
  switch (unit) {
    case 'em':
      return `${trim(em)}em`
    case 'px':
      return `${trim(em * sizePx)}px`
    case '%':
      return forCss ? `${trim(em)}em` : `${trim(em * 100)}%`
  }
}
