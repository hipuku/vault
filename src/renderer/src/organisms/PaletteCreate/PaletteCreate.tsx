...
import {
  faChevronLeft, faBarsStaggered, faTableColumns, faCheck,
  faPlus, faMinus, faMagnifyingGlass,
...
interface PaletteCreateProps {
  ...
  onCreateExpressive: (name: string, seeds: Array<{ hex: string; colourId: number | null }>, targetCount: number, strategy: FillStrategy) => Promise<Palette>
}
...
export function PaletteCreate({ library, onCancel, onCreateTonal, onCreateExpressive }: PaletteCreateProps): React.ReactElement {
  ...
  useEffect(() => {
    if (projectId == null) { setProjectColourIds(null); return }
    let live = true
    window.api.tag.listAssetIds('colour', projectId).then(ids => { if (live) setProjectColourIds(new Set(ids)) })
    return () => { live = false }
  }, [projectId])

  function resetSeeds(): void { setTonalSeed(null); setExpSeeds([]) }
  ...
  const nearIdentical = useMemo(() => {
    ...
    for (let i = 0; i < mids.length; i++)
      for (let j = i + 1; j < mids.length; j++)
        if (chroma.deltaE(mids[i], mids[j]) < 2.5) return true
    return false
  }, [expSwatches])
  ...
  const filtered = useMemo(
    () => q ? scopedColours.filter(c => c.name.toLowerCase().startsWith(q) || c.hex.toLowerCase().replace('#', '').startsWith(q.replace('#', ''))) : scopedColours,
    [scopedColours, q]
  )
  ...
  async function create(): Promise<void> {
    ...
    try {
      ...
      if (tab === 'tonal' && tonalSeed) {
        palette = await onCreateTonal(trimmed, tonalSeed.hex, tonalSeed.id, ALL_RAMPS.filter(r => ramps.has(r)))
        ...
        <div className={styles.controls}>
          ...
          <div className={styles.field}>
            <label className={styles.label}>
              {seedLabel}<span className={styles.labelHint}>{seedHint}</span>
            </label>
            ...
            {projectId == null ? (
              ...
              <>
                <div className={styles.search}>
                  ...
                  <input className={styles.searchInput} value={query} onChange={e => setQuery(e.target.value)} placeholder="Search colours…" spellCheck={false} />
                </div>
                  ...
                  {filtered.map(c => {
                    ...
                    return (
                      <button
                        ...
                        aria-pressed={on}
                      >
                        {on && <FontAwesomeIcon icon={faCheck} style={{ color: prefersDarkText(c.hex) ? '#000' : '#fff' }} />}
                      </button>
                    )
                  })}
            ...
            <>
              <div className={styles.field}>
                ...
                <div className={styles.stepper}>
                  <button type="button" className={styles.stepBtn} onClick={() => setTargetCount(c => Math.max(minCount, c - 1))} disabled={targetCount <= minCount}>
                    <FontAwesomeIcon icon={faMinus} />
                  </button>
                  <span className={styles.stepVal}>{Math.max(minCount, Math.min(targetCount, MAX_HUE_GROUPS))}</span>
                  <button type="button" className={styles.stepBtn} onClick={() => setTargetCount(c => Math.min(MAX_HUE_GROUPS, c + 1))} disabled={targetCount >= MAX_HUE_GROUPS}>
              ...
              <div className={styles.field}>
                ...
                <div className={styles.strategy}>
                  {STRATEGIES.map(s => (
                    <button key={s.id} type="button" className={[styles.pill, strategy === s.id ? styles.pillOn : ''].filter(Boolean).join(' ')} onClick={() => setStrategy(s.id)}>
                      {s.label}
                    </button>
                  ))}
                </div>
        ...
        <div className={styles.previewPane}>
          {!hasSeeds ? (
            <div className={styles.previewEmpty}>
              {tab === 'tonal' ? 'Pick a seed colour to preview the ramps.' : 'Pick at least one seed colour to preview the set.'}
            </div>
          ) : tab === 'tonal' ? (
            ...
            <>
              {nearIdentical && <p className={styles.advisory}>Some colours are nearly identical — try a lower hue count.</p>}
