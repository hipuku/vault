...
const RAW = {}
for (const m of tokenCss.matchAll(/^\s*(--[a-z0-9-]+):\s*([^;]+);/gmi)) RAW[m[1]] = m[2].trim()
...
export function pretty(value) {
  ...
  v = v.replace(/oklch\(\s*from\s+(oklch\([^)]*\)|#[0-9a-f]{3,8})\s+l\s+c\s+h\s*\/\s*([\d.]+)\s*\)/gi,
    (whole, base, alpha) => {
      try {
        const c = toRgb(base)
  ...
  v = v.replace(/oklch\([^)]*\)/gi, c => {
    try {
      ...
      if (!p) return c
      const n = x => Math.round(x * 255)
      return p.alpha !== undefined && p.alpha < 1
        ? `rgba(${n(p.r)}, ${n(p.g)}, ${n(p.b)}, ${p.alpha})`
        : formatHex(p)
    } catch { return c }
  })
